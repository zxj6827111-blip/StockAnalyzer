...
>
> **实施状态（2026-09-14）**：S1（引用级投影 `SnapshotRef`/`list_snapshot_refs`）、S2（选池改引用级 + 符号过滤下推 SQL）、
> S4 前半（行上限裁剪改协议约束、特征载荷只在最终 ≤cap 样本上取一次）已实现并测试；
> S3（manifest builder 内的第二段物化）与 S5（流式接口）未做；NAS 4 GiB 验收未做。
