...
每日条数 `bootstrap_max_rows_per_day`：>0 用配置值；**0 = 自动**，把整个 `max_rows`
预算平摊到全部可用决策日（`max_rows // 可用日数`），从而不需要人工试出一个魔数。

效果（就是本改动存在的原因）：

| | 保留决策日 | 总样本 |
|---|---|---|
| 旧 keep-last-N（40,000 / 334 每日） | ≈ 120 | 40,000 |
| 新 per_day 自动（40,000 / 480 日） | **480** | 39,840 |

总样本数不变 → **B4 的 4 GiB 内存前提不被破坏**（载荷只对最终 ≤cap 的样本物化，
见 `docs/learning_chain_b4_memory_plan_20260914.md`）。

按 `test_ratio=0.1`（config 实际值，非 0.2）估算：480 决策日 → test 名义 48 天
→ purge 后约 30~38 天 ≥ 20 ✓。旧口径 120 日 → test 名义 12 天 → purge 后 12 天 ✗。
...
- `calibration_ratio`/`test_ratio` 仍是 0.1。窗口能否稳过 20 天依赖 purge 后剩余日数，
  实测若余量仍薄，正解是继续扩大可用历史（两年 PIT 面板 2,405,272 行 / 482 交易日），
  **不是**下调窗口下限。
