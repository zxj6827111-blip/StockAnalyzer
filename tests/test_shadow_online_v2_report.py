...
def test_shadow_online_v2_report_builder_produces_return_calibration_and_execution_views(
    tmp_path: Path,
) -> None:
    (
        config,
    ...
    report = ShadowOnlineV2ReportBuilder(
        store=store,
    ...
    assert report.m11_v2_report["status"] in {"stable", "redline_breach", "no_data"}

    first_row = report.rows[0]
    assert 0.0 <= first_row.shadow_v2_probability <= 1.0
    assert first_row.shadow_v2_signal in {0, 1}


def test_shadow_online_v2_report_builder_keeps_reconciled_rows_before_label_mature_time(
    tmp_path: Path,
) -> None:
    (
        config,
    ...
    assert report.status == "updated"
    assert len(report.rows) >= 1
    assert report.run_result["samples_used"] >= 1
