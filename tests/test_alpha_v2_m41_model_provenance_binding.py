...
def _artifact(tmp_path: Path, *, commit: str = A, model_id: str = "epoch_test") -> Path:
    ...
    model = fit_frozen_model(
        ...
        provenance={
            ...
            # P0：v3 追加训练模式 + 双价格源身份（feature qfq / execution raw）。
            ...
            "feature_data_identity": {
                ...
                "fingerprint": "r41-fixture-fingerprint",
            ...
            "execution_data_identity": {
                ...
                "fingerprint": "r41-fixture-exec-fingerprint",
                "fingerprint_version": "v2",
                "source_window": ["2025-10-13", "2026-06-30"],
                "warmup_days": 200,
                "rows": 4321,
...
def _write_preflight_report(tmp_path: Path, *, artifact: Path, commit: str = A) -> Path:
    ...
    payload: dict[str, object] = {
        ...
        "data_identity": {
            ...
            "warmup_days": 200,
            "source_window": ["2025-10-13", "2026-06-30"],
            # P0：gate 还会逐项对账两条数据身份；报告按模型封存值如实落。
            "feature_data_identity": dict(model_identity.get("feature_data_identity") or {}),
            "execution_data_identity": dict(model_identity.get("execution_data_identity") or {}),
        },
