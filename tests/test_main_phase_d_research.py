...
def _load_test_config(base_dir: Path | None = None) -> StockAnalyzerConfig:
    ...
    external_temp_root = base_dir or Path(
        tempfile.mkdtemp(prefix="stock_analyzer_phase_d_research_")
    )
    temp_root = root / "tmp_phase_d_research" / external_temp_root.name
    shutil.rmtree(temp_root, ignore_errors=True)
    temp_root.mkdir(parents=True, exist_ok=True)
    relative_root = Path("tmp_phase_d_research") / external_temp_root.name
    ...
    config.evolution.suggestions_dir = str(
        Path("suggestions") / "test_phase_d_research" / external_temp_root.name
    )
...
def _prepare_phase_d_service(tmp_path: Path) -> StockAnalyzerService:
...
def test_service_phase_d_research_reports_run_and_persist(tmp_path: Path) -> None:
    service = _prepare_phase_d_service(tmp_path)
    ...
    assert int(alphalens["factor_count"]) >= 1

    assert shap["research_id"] == "shap_sidecar"
    assert Path(str(shap["output_path"])).exists() is True
    assert shap["status"] == "ok"

    assert catboost["research_id"] == "catboost_shadow"
    assert Path(str(catboost["output_path"])).exists() is True
    assert catboost["affects_main_model"] is False

    qlib_bundle = _as_mapping(qlib["bundle_paths"])
    assert qlib["research_id"] == "qlib_bridge"
    assert qlib["status"] == "ok"
    assert Path(str(qlib_bundle["manifest_path"])).exists() is True
    assert Path(str(qlib_bundle["dataset_path"])).exists() is True

    assert int(audit["records"]) >= 4


def test_service_phase_d_finbert_report_runs_and_persists(tmp_path: Path) -> None:
    service = _prepare_phase_d_service(tmp_path)

    payload = _as_mapping(
        service.build_phase_d_finbert_report(
            records=[
                {
                    "symbol": "600000.SH",
    ...
    assert payload["research_id"] == "finbert_sidecar"
    assert payload["status"] == "ok"
    assert Path(str(payload["output_path"])).exists() is True
